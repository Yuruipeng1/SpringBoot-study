...
class FilteredPropertySource extends PropertySource<PropertySource<?>> {
	...
	static void apply(ConfigurableEnvironment environment, String propertySourceName, Set<String> filteredProperties,
		...
		//这个是获取到当前程序的运行时环境。主要就是当前系统的环境变量、当前程序的环境变量等等
		MutablePropertySources propertySources = environment.getPropertySources();
		//这里是从运行时环境中查找key=defaultProperties的属性值，一般情况下，我们没有配置这个属性，
		// 获取到的original就是null,就会走到下面的if分支中，回到我们的lambda表达式中，我们就看这个分支吧
